...
public class AgentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all agents").get("/services/noospherehub/api/agents").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new agent")
                        .post("/services/noospherehub/api/agents")
                ...
                .repeat(5)
                .on(
                    exec(http("Get created agent").get("/services/noospherehub${new_agent_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created agent").delete("/services/noospherehub${new_agent_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
