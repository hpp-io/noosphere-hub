...
public class NodeContainerGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all nodeContainers")
                    .get("/services/noospherehub/api/node-containers")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new nodeContainer")
                        .post("/services/noospherehub/api/node-containers")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_nodeContainer_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created nodeContainer")
                            .get("/services/noospherehub${new_nodeContainer_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created nodeContainer")
                        .delete("/services/noospherehub${new_nodeContainer_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the NodeContainer entity").exec(scn);
