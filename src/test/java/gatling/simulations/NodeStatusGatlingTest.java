...
public class NodeStatusGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all nodeStatuses")
                    .get("/services/noospherehub/api/node-statuses")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new nodeStatus")
                        .post("/services/noospherehub/api/node-statuses")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"createdAt\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"lastKeepAliveAt\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_nodeStatus_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created nodeStatus").get("/services/noospherehub${new_nodeStatus_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created nodeStatus")
                        .delete("/services/noospherehub${new_nodeStatus_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the NodeStatus entity").exec(scn);
