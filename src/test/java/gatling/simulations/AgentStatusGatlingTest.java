...
/**
 * Performance test for the AgentStatus entity.
 *
...
public class AgentStatusGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all agentStatuses")
                    .get("/services/noospherehub/api/agent-statuses")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new agentStatus")
                        .post("/services/noospherehub/api/agent-statuses")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_agentStatus_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created agentStatus")
                            .get("/services/noospherehub${new_agentStatus_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created agentStatus")
                        .delete("/services/noospherehub${new_agentStatus_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AgentStatus entity").exec(scn);
