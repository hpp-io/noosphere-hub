 ...
 */
public class NodeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all nodes").get("/services/noospherehub/api/nodes").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new node")
                        .post("/services/noospherehub/api/nodes")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created node").get("/services/noospherehub${new_node_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created node").delete("/services/noospherehub${new_node_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Node entity").exec(scn);
