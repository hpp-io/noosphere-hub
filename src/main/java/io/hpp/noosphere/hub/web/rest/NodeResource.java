...
import io.hpp.noosphere.hub.service.NodeService;
...
/**
 * REST controller for managing {@link io.hpp.noosphere.hub.domain.Node}.
 */
@RestController
...
public class NodeResource {
    ...
    private static final String ENTITY_NAME = "noosphereHubNode";
    ...
    public ResponseEntity<List<NodeDTO>> getAllNodes(
        ...
        if ("nodestatus-is-null".equals(filter)) {
            LOG.debug("REST request to get all Nodes where nodeStatus is null");
            return new ResponseEntity<>(nodeService.findAllWhereNodeStatusIsNull(), HttpStatus.OK);
        }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<NodeDTO> getNode(@PathVariable("id") UUID id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteNode(@PathVariable("id") UUID id) {
