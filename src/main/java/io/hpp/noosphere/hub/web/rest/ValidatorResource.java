...
public class ValidatorResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new validatorDTO, or with status {@code 400 (Bad Request)} if the validator
     * has already an ID.
    ...
    public ResponseEntity<List<ValidatorDTO>> search(
      ...
      @org.springdoc.core.annotations.ParameterObject Pageable pageable
        ...
        Page<ValidatorDTO> page = validatorService.search(searchVm.getName(), searchVm.getStatusCode(), searchVm.getCreatedByUserId(), pageable);
    ...
    public ResponseEntity<ValidatorDTO> registerValidator(@Valid @RequestBody RegisterValidatorVm validatorVm) throws URISyntaxException {
        ...
        Instant now = Instant.now();
        ValidatorDTO validatorDTO = validatorService.register(validatorVm.getName(), validatorVm.getApiKey(), validatorVm.getWalletAddress(), validatorVm.getEmail(), now);
        return ResponseEntity.created(new URI("/api/validators/" + validatorDTO.getId()))
          .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, validatorDTO.getId().toString()))
          .body(validatorDTO);
    }
